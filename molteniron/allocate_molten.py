...
while flag == 1:
    try:
        status = subprocess.check_output(['bash', '-c', "molteniron -o result status|grep %s|awk -F '|' '{print $6}'" % nodepool])
        check = status.split()
        if 'ready' in check:
            ...
            if int(d['status']) == 200:
                ...
                user = subprocess.check_output(['bash', '-c', "molteniron get_field '%s' 'ipmi_user'" % owner_name])
                user_d = json.loads(user)
                hardware_info.write(" ")
                hardware_info.write(user_d['result'][0]['field'])
                pwd = subprocess.check_output(['bash', '-c', "molteniron get_field '%s' 'ipmi_password'" % owner_name])
